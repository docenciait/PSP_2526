...
#### 7. Retrofit2 + Compose para un CRUD real

1. **Modelo de flujo:** Sigue usando `PostViewModel` con `MutableStateFlow` y expón `posts` con `collectAsStateWithLifecycle()` desde Compose para que la UI se actualice al cargar los datos.
2. **Pantalla:** Crea un `@Composable fun PostScreen(viewModel: PostViewModel = hiltViewModel())` que muestra los posts en un `LazyColumn` y ofrece campos `TextField` y botones para crear, actualizar o eliminar.
3. **Acciones:** Llama a `viewModel.load()` en `LaunchedEffect(Unit)` para cargar los datos al inicio. Toca un botón para llamar a `viewModel.add(...)`, `viewModel.update(...)` o `viewModel.remove(...)`, y deja que el estado se actualice automáticamente.
4. **Manejo de estados:** Usa `remember { mutableStateOf(PostForm()) }` para guardar los valores actuales del formulario y `rememberCoroutineScope()` si necesitas disparar operaciones que no dependan del `viewModelScope` (aunque en general conviene dejar que el ViewModel maneje el ciclo de vida).
5. **Feedback visual:** Añade `Snackbar` o `LoadingState` con `remember { mutableStateOf(false) }` para mostrar el resultado del CRUD y mostrar errores (usa `Result` o `Either` para capturar fallos y evitar que Compose lance excepciones).

Ejemplo simplificado de UI:
    ...
    Scaffold(scaffoldState = scaffoldState) {
        Column(Modifier.padding(16.dp)) {
            LazyColumn(modifier = Modifier.weight(1f)) {
                items(posts) { post ->
                    PostItem(post, onDelete = { viewModel.remove(post.id!!)} )
                }
            }
            ...
            Row {
                Button(onClick = { viewModel.add(title, body) }) { Text("Crear") }
                Spacer(Modifier.width(8.dp))
                Button(onClick = { /* muestra un diálogo para seleccionar un id y llamar update */ }) {
                    Text("Actualizar")
                }
            }
        }
...
Para completar el CRUD, extiende la UI con componentes que permitan seleccionar el `Post` activo (por ejemplo, al hacer clic en un elemento) y luego pasar sus datos al formulario para editar o borrar. Mantén la lógica de red dentro de `PostViewModel` y usa Compose únicamente para presentar el estado y disparar eventos de usuario.
